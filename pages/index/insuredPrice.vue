	...
	<view class="type-con">
		...
		<view class="type-item-list" @click="goChecke(i, item)" :class="actvieIndex==i?'active':''" v-for="(item, i) in platformList" :key="i">
			<view class="company">
				...
				<view v-if="code==item.companyCode&&isInsurance" class="note" style="color: #FF0000;">大件3.60最低保费3元</view>
			</view>
			<view class="price">
...
<script>
	export default {
		...
		onLoad(options) {
			...
			that.isInsurance = options.isInsurance=="true"?true:false;
			that.isPrice = that.type == "small" && options.isInsurance!="true" ? true : false;
