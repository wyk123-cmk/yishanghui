	...
	<view class="page-bg">
		...
		<form class="form">
			...
			<label class="label-item" @click="goInsuredPrice('')">
				<view>
					<text class="input-txt-bj" v-if="insuredPrice==null">
						{{expressCompany.code=="db001"&&type=="small"?"未保价物品最高可获得3倍运费,建议保价":"建议足额保价"}}
					</text>
		...
		<view class="estimate-price-box" v-if="estimatePriceList.length != 0">
			产品类型
			<view class="estimate-list" v-for="(item, k) in estimatePriceList" :key="k" @click="btnSelProductType(item, k, true)">
				<view class="con" :class="selectProduct==k&&item.isActiveClass?'active':''">
					<view class="name">
						<view class="flex">
							{{item.productName}}
...
<script>
	...
	export default {
		...
		methods: {
			...
			//预估价格
			btnSelProductType: function(item, i, bool) {
				let that = this;
			...
			//请填写报价信息
			goInsuredPrice: function(isInsurance) {
