...
<template>
	<view>
		...
		<!-- 我的寄件 -->
		<scroll-view class="tab-con" v-if="tabIndex==1" scroll-y="true" @scrolltolower="sentScroll">
			<view v-if="sentList.length==0" class="none-data-list"><image src="../../static/images/none-jj.svg"></image>暂无内容</view>
			<view v-else class="order-item-list" v-for="(item, index) in sentList" :key="index">
				...
				<view v-if="item.status=='4'" class="finished-time">
				...
				<view v-if="item.status=='1'" class="info">
					...
					<text class="btn del" @click="goPage(3,item.mastCode)">取消</text>
				</view>
		...
		<!-- 我的收件 -->
		<scroll-view class="tab-con" v-if="tabIndex==2" scroll-y="true" @scrolltolower="receiveScroll">
			<view v-if="receiveList.length==0" class="none-data-list"><image src="../../static/images/none-sj.svg"></image>暂无内容</view>
			<view v-else class="order-item-list" v-for="(item, index) in receiveList" :key="index">
				<view class="number" @click="setClip(item.expressCode)">运单号：{{item.expressCode}}<text class="iconfont icon-copy"></text></view>
				<text class="detail" @click="goPage(1,item.mastCode)">详情</text>
				<view class="transport" @click="goPage(2,item.mastCode)">
					<view class="express">
						<text class="city">{{item.expressFrom.areaInfo.city}}</text>
						<text>{{item.expressFrom.name}}</text>
					</view>
