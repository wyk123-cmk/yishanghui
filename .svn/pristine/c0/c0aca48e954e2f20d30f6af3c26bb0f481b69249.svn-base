...
<template>
	<view class="page-bg">
		...
		<view class="tab-con">
			<view v-if="couponData.length == 0  " class="none-data-list"><image src="../../static/images/none.svg"></image>暂无内容</view>
			<view v-else v-for="(item, index) in couponData" :key="index" class="coupon-list" :class="[item.isget=='1'&&item.status=='2'?'none':'']">
				...
				<text class="btn" v-if="(item.status=='2' && item.isget.indexOf('_')<0)|| ( item.isget=='0'&& item.status=='3') || (item.isget=='0'&& item.status=='1')" @click="postCoupon(item.id,item.isget)">立即领取</text>
				<!-- #ifdef H5 -->
		...
		<view v-if="showSend"  @click="showSend=false">
			...
			<view class="share-popup"><image src="../../static/images/share.svg" alt="请点击右上角的分享按钮分享"></image></view>
		</view>
	...
	methods: {
		...
		postCoupon: function(id, isget){
			...
			that.$util.networkRequest({
				...
				method: "POST",
				data: {userType: that.userType, couponId: id},
				success: res => {
					if (res !== "") {
		...
		btnNavTab: function(key){
			this.userType = key;
			this.getCoupon();
			this.coupon=key=="comp"?'企业':'我的'
			
		},
		getCoupon: function(){
			const that = this;
			let type = that.userType;
